...
function initGestureControl(ref) {
  let lastGesture = "";
  ...
  function pointingUP(results, nowInMs) {
    ...
    if (lastGesture !== 'Pointing_Up' || nowInMs - lastGestureTime > 300) {
      lastPos = undefined;
      movingDis = 0;
    }
    if (lastPos !== undefined) {
      movingDis += indexPos.x - lastPos.x;
      if (movingDis > 0.2) {
        ref.selectCard(1);  
        movingDis = 0;
      }
...
window.initGestureControl = initGestureControl;